...
// Leaders
for l in list.Range(0, args.leaders, 1) {
	// Followers
	for f in list.Range(0, args.replicas+1, 1) {
		containers: {
			"redis-\(l)-\(f)": {
...
let leaderConfigTemplate = localData.redis.commonConfig & localData.redis.leaderConfig & args.leaderConfig
let followerConfigTemplate = localData.redis.commonConfig & localData.redis.followerConfig & args.followerConfig
localData: {
	redis: {
		commonConfig: {
